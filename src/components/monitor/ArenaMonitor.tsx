...
import {
  ...
  Cpu, BarChart3, ChevronDown, ChevronRight, Send,
...
interface PipelineState {
  ...
  nodes: {
    classifier: NodeStatus;
    queryAnalyzer: NodeStatus;
    webResearch: NodeStatus;
    contextBuilder: NodeStatus;
    contextValidator: NodeStatus;
    personaLoader: NodeStatus;
    personaLoop: NodeStatus;
...
const NODE_LABELS: Record<string, string> = {
  ...
  personaLoop: 'Persona Loop',
  aggregator: 'Aggregator',
};

const STATUS_STYLES: Record<NodeStatus, { border: string; bg: string; icon: React.ReactNode }> = {
  idle: {
    border: 'border-zinc-800/50',
    bg: 'bg-zinc-900/40',
    icon: <div className="w-2 h-2 rounded-full bg-zinc-700" />,
  },
  running: {
  ...
  skipped: {
    ...
    bg: 'bg-zinc-950/40',
    icon: <div className="w-2 h-2 rounded-full bg-zinc-800" />,
  },
};
...
function FlowNode({
  ...
  const relevantLogs = logs.filter(l => l.step === nodeKey || l.step === nodeKey.replace(/([A-Z])/g, '_$1').toLowerCase());
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        'w-full text-left rounded-xl border p-3.5 transition-all duration-300 group',
        style.border, style.bg,
        isSelected
          ? 'ring-1 ring-violet-500/30 shadow-lg shadow-violet-500/5'
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {lastLog && (
            <p className="text-[10px] text-zinc-500 truncate mt-0.5">{lastLog.message}</p>
          )}
        </div>
        {relevantLogs.length > 0 && (
          <span className="text-[9px] text-zinc-600 tabular-nums">{relevantLogs.length}</span>
        )}
      </div>
      {extra}
    </button>
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/[0.04]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] text-zinc-600 tabular-nums">{filtered.length}</span>
        </div>
        <button
      ...
      >
        {filtered.length === 0 ? (
          <p className="text-[11px] text-zinc-700 text-center py-8">Nenhum log ainda...</p>
        ) : (
          filtered.map(log => (
            <LogLine key={log.id} log={log} levelColors={levelColors} />
          ))
        )}
      </div>
    </div>
  ...
  return (
    <div className="group">
      ...
      >
        ...
        <span className="text-[10px] text-zinc-600 w-24 shrink-0 truncate mt-px">{log.step}</span>
        <span className="text-[10px] text-zinc-300 flex-1 break-words">{log.message}</span>
        {hasDetail && (
          expanded
      ...
      {expanded && hasDetail && (
        <pre className="ml-[5.5rem] text-[9px] text-zinc-500 bg-zinc-950/60 rounded-lg p-3 my-1 overflow-x-auto max-h-60 border border-white/[0.03]">
  ...
  if (batches.length === 0) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-[11px] text-zinc-700">Nenhum batch processado ainda...</p>
      </div>
    );
  }
                ...
                <table className="w-full text-[10px]">
                  <thead>
                    <tr className="text-zinc-600 font-semibold uppercase tracking-wider border-b border-white/[0.04]">
                      ...
                      <th className="text-center px-3 py-2 w-16">Sentiment</th>
                  ...
                  <tbody>
                    {batch.personas_summary.map((p, pi) => (
                      <tr key={pi} className="border-b border-white/[0.02] hover:bg-white/[0.02]">
                        ...
                        <td className="px-3 py-1.5 text-zinc-400 max-w-xs truncate">{p.comment}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function ProgressStats({ progress, startTime, endTime }: {
  ...
  return (
    <div className="grid grid-cols-5 gap-2 px-4 py-3">
      {[
        { label: 'Processadas', value: `${progress.processed.toLocaleString('pt-BR')}/${progress.total.toLocaleString('pt-BR')}`, color: 'text-white' },
        ...
        { label: 'Vel.', value: `${rate}/s`, color: 'text-sky-400' },
      ].map(s => (
...
export function ArenaMonitor() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    addLog('system', 'info', `Pipeline iniciado: "${q}"`);

    // Classify route first
    updateNode('classifier', 'running');
    addLog('classifier', 'info', 'Classificando rota...');
    ...
    try {
      ...
      if (classifyRes.ok) {
        ...
        const route = classification.route || 'python';
        updateNode('classifier', 'complete');
        addLog('classifier', 'info', `Rota: ${route.toUpperCase()}`, classification);
        setState(prev => ({ ...prev, route }));

        if (route === 'local') {
          addLog('system', 'info', 'Processamento local via banco de dados — sem Python');
          // Mark all Python nodes as skipped
          ['queryAnalyzer', 'webResearch', 'contextBuilder', 'contextValidator'].forEach(n => updateNode(n, 'skipped'));
          updateNode('personaLoader', 'complete');
          updateNode('personaLoop', 'skipped');
        ...
        addLog('classifier', 'error', `HTTP ${classifyRes.status}`);
      }
    } catch (err: any) {
      if (err?.name === 'AbortError') return;
      updateNode('classifier', 'error');
      addLog('classifier', 'error', err?.message || 'Erro desconhecido');
    }
    ...
    addLog('system', 'info', 'Conectando ao backend Python...');
    ...
    try {
      ...
      addLog('system', 'info', 'Stream SSE conectado');
      ...
      addLog('system', 'info', 'Stream SSE finalizado');
    } catch (err: any) {
      if (err?.name === 'AbortError') return;
      addLog('system', 'error', `Erro: ${err?.message}`);
    }

    setState(prev => ({ ...prev, endTime: Date.now() }));
    setRunning(false);

    function processSSEEvent(payload: { type: string; data: any }) {
      switch (payload.type) {
        case 'phase': {
          const phase = payload.data?.phase;
          const phaseMap: Record<string, { node: string; prevNodes?: string[] }> = {
            analyzing_query: { node: 'queryAnalyzer' },
            web_research: { node: 'webResearch', prevNodes: ['queryAnalyzer'] },
            building_context: { node: 'contextBuilder', prevNodes: ['webResearch'] },
            loading_personas: { node: 'personaLoader', prevNodes: ['queryAnalyzer', 'webResearch', 'contextBuilder', 'contextValidator'] },
            processing_personas: { node: 'personaLoop', prevNodes: ['personaLoader'] },
            aggregating: { node: 'aggregator', prevNodes: ['personaLoop'] },
          };

          const mapping = phaseMap[phase];
          if (mapping) {
            // Complete previous nodes
            mapping.prevNodes?.forEach(n => {
              setState(prev => {
                if (prev.nodes[n as keyof typeof prev.nodes] === 'running') {
                  return { ...prev, nodes: { ...prev.nodes, [n]: 'complete' } };
                }
                return prev;
              });
            });
            updateNode(mapping.node, 'running');
            addLog(mapping.node, 'info', payload.data?.message || `Phase: ${phase}`);
          }
          break;
        }
        ...
        case 'web_complete': {
          ...
          addLog('webResearch', 'info', `${payload.data.snippets_count} snippets, ${payload.data.sources_count} fontes`, payload.data);
          break;
        }

        case 'personas_loaded': {
          updateNode('personaLoader', 'complete');
          addLog('personaLoader', 'info', `${payload.data.count?.toLocaleString('pt-BR')} personas carregadas`, payload.data);
          setState(prev => ({
            ...prev,
        ...
        case 'results': {
          ...
          addLog('aggregator', 'info', 'Resultados finais agregados', {
            total: payload.data.total,
            positive: payload.data.positive,
            negative: payload.data.negative,
            neutral: payload.data.neutral,
            comments_count: payload.data.comments?.length || 0,
          });
          break;
        }

        case 'done': {
          addLog('system', 'info', `Pipeline concluido em ${(payload.data.processing_time_ms / 1000).toFixed(1)}s`, payload.data);
          // Complete all remaining running nodes
          setState(prev => {
  ...
  return (
    <div className="h-screen flex flex-col bg-black overflow-hidden">
      ...
      <div className="shrink-0 border-b border-white/[0.06] bg-zinc-950/80 backdrop-blur-xl px-6 py-4">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-sm font-bold text-white tracking-tight">Arena Monitor</h1>
              <p className="text-[10px] text-zinc-600">Pipeline de analise em tempo real</p>
            </div>
          </div>
          ...
          <div className="flex-1 flex items-center gap-2 max-w-2xl">
            <div className="flex-1 relative">
              <input
                ...
                placeholder="Digite uma pergunta para monitorar..."
                disabled={running}
                className="w-full px-4 py-2.5 bg-white/[0.04] hover:bg-white/[0.06] border border-white/[0.08] focus:border-violet-500/50 rounded-xl text-sm text-white placeholder:text-zinc-600 outline-none focus:ring-2 focus:ring-violet-500/20 transition-all duration-200 disabled:opacity-50"
              />
            </div>
            {running ? (
              <button
          ...
          <div className="flex items-center gap-3 text-[10px]">
            {state.route !== 'unknown' && (
              <span className={cn(
                ...
                {state.route === 'python' ? 'Python AI' : 'Local DB'}
              </span>
            )}
            {state.startTime && (
      ...
      <div className="flex-1 flex min-h-0">
        {/* ─── Left: Flow Nodes ─── */}
        <div className="w-72 shrink-0 border-r border-white/[0.06] bg-zinc-950/40 overflow-y-auto py-4 px-3 space-y-0 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-zinc-800/50">
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-600 px-2 mb-3">Pipeline Flow</p>
          {nodeOrder.map((key, i) => (
            <div key={key}>
              <FlowNode
                nodeKey={key}
                status={state.nodes[key as keyof typeof state.nodes]}
                logs={state.logs}
              ...
              {i < nodeOrder.length - 1 && (
                <FlowArrow active={state.nodes[key as keyof typeof state.nodes] === 'complete' || state.nodes[key as keyof typeof state.nodes] === 'running'} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="shrink-0 flex items-center gap-1 px-4 py-2 border-b border-white/[0.04]">
            {(['logs', 'batches'] as const).map(tab => (
              ...
              >
                {tab === 'logs' ? 'Logs' : `Batches (${state.batches.length})`}
              </button>
            ))}
          </div>
