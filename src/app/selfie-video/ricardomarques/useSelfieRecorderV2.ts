...
/**
 * V2 Recorder hook — isolated from V1.
 * Uses /api/v2/* endpoints and v2_* tables.
 */
...
export interface UseSelfieRecorderV2Options {
  slug: string;
  maxSeconds?: number;
  minSeconds?: number;
}

export function useSelfieRecorderV2({
  slug,
  maxSeconds = 20,
  minSeconds = 3,
}: UseSelfieRecorderV2Options) {
  ...
  async function handleEnviar() {
    ...
    try {
      ...
      await fetch('/api/v2/confirm-upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
